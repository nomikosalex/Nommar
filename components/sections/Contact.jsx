...
export default function Contact() {
  ...
  return (
    <div>
      <Reveal style="text-align:center;padding:clamp(64px,8vw,112px) clamp(24px,6vw,40px) clamp(10px,2vw,30px);">
        <div style={css("font-family:var(--font-jost),sans-serif;font-size:12px;letter-spacing:0.34em;text-transform:uppercase;color:#C2A56B;margin-bottom:18px;")}>{t.contactEyebrow}</div>
        <h1 style={css("font-family:var(--font-cinzel),serif;font-weight:500;font-size:clamp(32px,5vw,58px);letter-spacing:0.05em;color:#3D2F25;margin:0;")}>{t.navContact}</h1>
        <p style={css("font-family:var(--font-cormorant),serif;font-style:italic;font-size:clamp(18px,2vw,24px);color:#8A7965;margin:18px auto 0;max-width:36ch;")}>{t.contactSub}</p>
      </Reveal>
      ...
      <section style={css('max-width:1200px;margin:0 auto;padding:clamp(36px,5vw,64px) clamp(24px,6vw,72px);display:grid;grid-template-columns:repeat(auto-fit,minmax(min(360px,100%),1fr));gap:clamp(36px,4vw,64px);')}>
        <Reveal direction="right">
          <h2 style={css("font-family:var(--font-cinzel),serif;font-weight:500;font-size:24px;letter-spacing:0.03em;color:#3D2F25;margin:0 0 26px;")}>{t.getInTouch}</h2>
          <div style={css('display:flex;flex-direction:column;gap:24px;')}>
            <div>
              <div style={css("font-family:var(--font-jost),sans-serif;font-size:10.5px;letter-spacing:0.24em;text-transform:uppercase;color:#C2A56B;margin-bottom:7px;")}>{t.labelLocation}</div>
              <div style={css("font-family:var(--font-jost),sans-serif;font-weight:300;font-size:16px;color:#3D2F25;line-height:1.6;")}>{t.locCity}<br />{t.locRegion}</div>
            </div>
            <div>
            ...
            <div>
              <div style={css("font-family:var(--font-jost),sans-serif;font-size:10.5px;letter-spacing:0.24em;text-transform:uppercase;color:#C2A56B;margin-bottom:7px;")}>{t.labelHours}</div>
              <div style={css("font-family:var(--font-jost),sans-serif;font-weight:300;font-size:16px;color:#3D2F25;line-height:1.7;")}>{t.hoursLine}<br />{t.byAppointment}</div>
            </div>
          </div>
        ...
        <Reveal direction="left">
          <div style={css('background:#FFFDF8;border:1px solid rgba(194,165,107,0.3);padding:clamp(28px,3.4vw,46px);box-shadow:0 26px 56px -36px rgba(61,47,37,0.5);text-align:center;')}>
            ...
            <h2 style={css("font-family:var(--font-cinzel),serif;font-weight:500;font-size:24px;letter-spacing:0.03em;color:#3D2F25;margin:0 0 12px;")}>{t.reserveRitual}</h2>
            <p style={css("font-family:var(--font-cormorant),serif;font-style:italic;font-size:20px;line-height:1.55;color:#6E5E50;margin:0 auto 30px;max-width:34ch;")}>{t.reserveRitualDesc}</p>
