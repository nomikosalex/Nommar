...
import { useAdminLocale, setAdminLocale } from '@/lib/useAdminLocale';
...
export default function AdminHeader() {
  ...
  const langBtn = (code, label) => (
    <FX
      ...
      style={'font-family:var(--font-jost),sans-serif;font-size:11px;letter-spacing:0.12em;background:none;border:none;cursor:pointer;padding:2px 4px;color:' + (locale === code ? '#3D2F25;font-weight:600;' : '#A8967C;')}
      hover="color:#C2A56B;"
    >
      {label}
    </FX>
      ...
      <div style={css('max-width:1180px;margin:0 auto;padding:16px clamp(18px,4vw,40px);display:flex;align-items:center;justify-content:space-between;gap:20px;')}>
        <div style={css('display:flex;align-items:center;gap:26px;')}>
          ...
          <nav style={css('display:flex;flex-wrap:wrap;gap:18px;')}>
            {link('/admin', 'Bookings')}
            {link('/admin/new-booking', locale === 'gr' ? 'Γρήγορη Κράτηση' : 'Quick Booking')}
        ...
        <div style={css('display:flex;align-items:center;gap:14px;')}>
          <div style={css('display:flex;align-items:center;gap:4px;')}>
            {langBtn('en', 'EN')}
            <span style={css('color:#D8CBB6;font-size:11px;')}>/</span>
            {langBtn('gr', 'ΕΛ')}
          </div>
          <FX as="button" onClick={logout} style="font-family:var(--font-jost),sans-serif;font-size:11px;letter-spacing:0.18em;text-transform:uppercase;color:#8A7965;background:none;border:1px solid rgba(194,165,107,0.45);padding:9px 16px;cursor:pointer;border-radius:1px;" hover="border-color:#C2A56B;color:#3D2F25;">{locale === 'gr' ? 'Αποσύνδεση' : 'Log out'}</FX>
        </div>
      </div>
    </header>
  );
