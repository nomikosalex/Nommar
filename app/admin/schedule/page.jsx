...
const fmt = (locale) => new Intl.DateTimeFormat(locale === 'gr' ? 'el-GR' : 'en-GB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit', timeZone: TZ });
...
const COPY = {
  en: {
    heading: 'Schedule', subtitle: 'Therapists, the treatments they perform, working hours and time off — all feed availability. Athens time.',
    loading: 'Loading…', therapists: 'Therapists', newTherapistPlaceholder: 'New therapist name', addTherapist: '+ Add therapist',
    timeOff: 'Time Off / Closures', deactivate: 'Deactivate', activate: 'Activate', delete: 'Delete',
    performs: 'Performs', workingHours: 'Working hours', noneSet: 'None set.', addHours: 'Add hours',
    noUpcoming: 'No upcoming time off.', pickStartEnd: 'Pick a start and end.', couldNotAdd: 'Could not add.',
    wholeSpa: 'Whole spa', reasonPlaceholder: 'Reason (optional)', addTimeOff: 'Add time off', remove: 'Remove',
    weekdays: ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'],
    weekdaysShort: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'],
  },
  gr: {
    heading: 'Πρόγραμμα', subtitle: 'Θεραπευτές, οι θεραπείες που εκτελούν, ώρες εργασίας και άδειες — όλα τροφοδοτούν τη διαθεσιμότητα. Ώρα Αθήνας.',
    loading: 'Φόρτωση…', therapists: 'Θεραπευτές', newTherapistPlaceholder: 'Όνομα νέου θεραπευτή', addTherapist: '+ Προσθήκη θεραπευτή',
    timeOff: 'Άδειες / Κλείσιμο', deactivate: 'Απενεργοποίηση', activate: 'Ενεργοποίηση', delete: 'Διαγραφή',
    performs: 'Εκτελεί', workingHours: 'Ώρες εργασίας', noneSet: 'Δεν έχουν οριστεί.', addHours: 'Προσθήκη ωρών',
    noUpcoming: 'Καμία προσεχής άδεια.', pickStartEnd: 'Επιλέξτε ώρα έναρξης και λήξης.', couldNotAdd: 'Δεν ήταν δυνατή η προσθήκη.',
    wholeSpa: 'Όλο το σπα', reasonPlaceholder: 'Αιτία (προαιρετικό)', addTimeOff: 'Προσθήκη άδειας', remove: 'Αφαίρεση',
  ...
  return (
    <>
      ...
      <main style={css('max-width:1100px;margin:0 auto;padding:clamp(28px,4vw,48px) clamp(18px,4vw,40px);')}>
        ...
        <p style={css("font-family:var(--font-jost),sans-serif;font-size:13px;color:#8A7965;margin:0 0 28px;")}>{t.subtitle}</p>

        <div style={css(eyebrow + 'margin-bottom:14px;')}>{t.therapists}</div>
        {staff === null ? (
          <p style={css("font-family:var(--font-cormorant),serif;font-style:italic;font-size:18px;color:#8A7965;")}>{t.loading}</p>
        ) : (
          <div style={css('display:flex;flex-direction:column;gap:16px;')}>
            {staff.map((s) => <StaffCard key={s.id} staff={s} services={services} onPatch={patchStaff} onDelete={delStaff} reload={load} t={t} />)}
          </div>
        )}
        <div style={css('display:flex;gap:8px;align-items:center;margin-top:16px;')}>
          <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder={t.newTherapistPlaceholder} style={css(inp + 'min-width:200px;')} />
          <FX as="button" onClick={addStaff} style={addBtn} hover="transform:translateY(-1px);">{t.addTherapist}</FX>
        </div>

        <div style={css(eyebrow + 'margin:44px 0 14px;')}>{t.timeOff}</div>
        <BlocksPanel staff={staff || []} blocks={blocks} onChange={load} t={t} locale={locale} />
      </main>
    </>
  );
}

function StaffCard({ staff, services, onPatch, onDelete, reload, t }) {
    ...
    <div style={css(card + 'padding:18px 20px;' + (staff.active ? '' : 'opacity:0.6;'))}>
      <div style={css('display:flex;flex-wrap:wrap;gap:12px;align-items:center;justify-content:space-between;margin-bottom:14px;')}>
        ...
        <div style={css('display:flex;gap:8px;')}>
          <FX as="button" onClick={() => onPatch({ id: staff.id, active: !staff.active })} style={ghost} hover="border-color:#C2A56B;color:#3D2F25;">{staff.active ? t.deactivate : t.activate}</FX>
          <FX as="button" onClick={() => onDelete(staff.id)} style="font-family:var(--font-jost),sans-serif;font-size:10.5px;letter-spacing:0.14em;text-transform:uppercase;color:#9B4444;background:none;border:1px solid rgba(155,68,68,0.4);padding:8px 12px;cursor:pointer;border-radius:1px;" hover="border-color:#9B4444;">{t.delete}</FX>
        </div>
      </div>

      <div style={css("font-family:var(--font-jost),sans-serif;font-size:10px;letter-spacing:0.18em;text-transform:uppercase;color:#A8967C;margin-bottom:8px;")}>{t.performs}</div>
      ...
      <div style={css("font-family:var(--font-jost),sans-serif;font-size:10px;letter-spacing:0.18em;text-transform:uppercase;color:#A8967C;margin-bottom:8px;")}>{t.workingHours}</div>
      <div style={css('display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px;')}>
        {staff.workingHours.length === 0 && <span style={css("font-family:var(--font-jost),sans-serif;font-size:13px;color:#8A7965;")}>{t.noneSet}</span>}
        {staff.workingHours.map((w) => (
          <span key={w.id} style={css("display:inline-flex;align-items:center;gap:8px;font-family:var(--font-jost),sans-serif;font-size:12.5px;color:#3D2F25;background:#F3EADA;border:1px solid rgba(194,165,107,0.35);padding:7px 12px;border-radius:2px;")}>
            {t.weekdaysShort[w.weekday]} {minToTime(w.startMin)}–{minToTime(w.endMin)}
      ...
      <div style={css('display:flex;flex-wrap:wrap;gap:8px;align-items:center;')}>
        <select value={weekday} onChange={(e) => setWeekday(e.target.value)} style={css(inp + 'cursor:pointer;')}>{t.weekdays.map((d, i) => <option key={i} value={i}>{d}</option>)}</select>
        ...
        <FX as="button" onClick={addHours} style={addBtn} hover="transform:translateY(-1px);">{t.addHours}</FX>
      </div>
    </div>
  );
}

function BlocksPanel({ staff, blocks, onChange, t, locale }) {
  ...
  const add = async () => {
    setError('');
    if (!startsAt || !endsAt) return setError(t.pickStartEnd);
    ...
    if (!r.ok) { const d = await r.json(); setError(d.error || t.couldNotAdd); return; }
  ...
  return (
    <div style={css(card + 'padding:20px 22px;')}>
      <div style={css('display:flex;flex-direction:column;gap:10px;margin-bottom:18px;')}>
        {blocks.length === 0 && <span style={css("font-family:var(--font-jost),sans-serif;font-size:13px;color:#8A7965;")}>{t.noUpcoming}</span>}
        {blocks.map((b) => (
          <div key={b.id} style={css('display:flex;align-items:center;justify-content:space-between;gap:12px;background:#F3EADA;border:1px solid rgba(194,165,107,0.3);padding:10px 14px;border-radius:2px;')}>
            <span style={css("font-family:var(--font-jost),sans-serif;font-size:13px;color:#3D2F25;")}><strong>{b.staff ? b.staff.name : t.wholeSpa}</strong> · {dt.format(new Date(b.startsAt))} → {dt.format(new Date(b.endsAt))}{b.reason ? ` · ${b.reason}` : ''}</span>
            <button onClick={() => remove(b.id)} style={css(delBtn)}>{t.remove}</button>
          </div>
        ))}
      </div>
      ...
      <div style={css('display:flex;flex-wrap:wrap;gap:8px;align-items:center;')}>
        <select value={staffId} onChange={(e) => setStaffId(e.target.value)} style={css(inp + 'cursor:pointer;')}>
          <option value="">{t.wholeSpa}</option>
        ...
        <input type="text" placeholder={t.reasonPlaceholder} value={reason} onChange={(e) => setReason(e.target.value)} style={css(inp + 'min-width:150px;')} />
        <FX as="button" onClick={add} style={addBtn} hover="transform:translateY(-1px);">{t.addTimeOff}</FX>
      </div>
    </div>
  );
}
