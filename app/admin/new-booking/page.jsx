...
const COPY = {
  en: {
    ...
    guestName: 'Guest name', phone: 'Phone', email: 'Email (optional)', language: 'Guest language', notes: 'Notes (optional)',
    ...
    required: 'Guest name, phone, therapist, service and time are all required.',
  },
  gr: {
    ...
    guestName: 'Όνομα πελάτη', phone: 'Τηλέφωνο', email: 'Email (προαιρετικό)', language: 'Γλώσσα πελάτη', notes: 'Σημειώσεις (προαιρετικό)',
    ...
    required: 'Όνομα, τηλέφωνο, θεραπευτής, θεραπεία και ώρα είναι υποχρεωτικά.',
  },
};
...
export default function NewBooking() {
  ...
  const chosenService = services.find((s) => s.slug === serviceSlug);
  ...
  const submit = async () => {
    setError('');
    if (!name.trim() || !phone.trim() || !staffId || !serviceSlug || !time) { setError(t.required); return; }
    setSubmitting(true);
